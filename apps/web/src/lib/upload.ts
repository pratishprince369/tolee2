...
export async function uploadFile(
  ...
  const { apiKey, timestamp, signature, cloudName, eager, eager_async, index: activeIndex } = sigData;
  ...
  if (isVideo && eager) {
    ...
    formData.append('eager_async', eager_async || 'true');
  }
  ...
  try {
    ...
    // 3. Optimize secure_url (Auto-Compression & Formatting)
    let optimizedUrl = uploadData.secure_url;
    if (isVideo && uploadData.eager && uploadData.eager.length > 0) {
      // Use the eager stream URL (.m3u8 or optimized MP4) generated by Cloudinary
      optimizedUrl = uploadData.eager[0].secure_url;
